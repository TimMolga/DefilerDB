import config, sqlite3 as sql, pandas as pd
...
class DefilerDB():
    ...
    def _create_insert_cursor_query(self, table_name, column_names, row):
        ...
        return f'INSERT INTO {table_name} ({column_names}) VALUES ({list_row_formatted})'
    ...
    def execute_create_table_query(self, table_name, column_names_types):
        ...
        """
        try:
            self._db_connection.execute(f'CREATE TABLE {table_name} ({column_names_types})')
        ...
        try:
            if not dataframe.empty:
                for row in dataframe.itertuples():
                    self._db_cursor.execute(self._create_insert_cursor_query(table_name, column_names, row))               
    ...
    def execute_select_query(self, query_string):
        ...
        try:
            if 'SELECT' in query_string:
                dataframe = pd.read_sql_query(query_string, self._db_connection)    
                return dataframe
            else:
